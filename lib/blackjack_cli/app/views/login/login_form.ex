defmodule BlackjackCli.Views.Login.LoginForm do
  ...
  def start_link(_) do
    GenServer.start_link(__MODULE__, :ok, name: BlackjackCli.via_tuple(@registry, :login))
  end
  ...
  def get_field(field) do
    GenServer.call(BlackjackCli.via_tuple(@registry, :login), {:get_field, field})
  end
  ...
  @spec get_fields :: map()
  def get_fields() do
    GenServer.call(BlackjackCli.via_tuple(@registry, :login), {:get_fields})
  end
  ...
  def update_field(field, input) do
    GenServer.call(BlackjackCli.via_tuple(@registry, :login), {:update_field, field, input})
  end
  ...
  @spec close_form :: :ok
  def close_form do
    GenServer.stop(BlackjackCli.via_tuple(@registry, :login), :normal)
  end
