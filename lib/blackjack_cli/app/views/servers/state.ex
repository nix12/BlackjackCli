defmodule BlackjackCli.Views.Servers.State do
  ...
  def update(model, msg) do
    case msg do
      ...
      {:event, %{ch: ?w}} ->
        update_cmd(%{model | input: max(model.input - 1, 0), data: model.data["servers"]})

      {:event, %{key: @down}} ->
        if model.menu do
          %{model | input: min(model.input + 1, length(menu()) - 1), data: model.data["servers"]}
        else
          update_cmd(%{
            model
            | input: min(model.input + 1, length(model.data["servers"]) - 1)
          })
        end
      ...
      {:event, %{key: @enter}} ->
        if model.menu == false do
          %{"server_name" => server_name} = match_servers(model.data["servers"], model.input)

          BlackjackCli.join_server(model.user, server_name)
  ...
  defp update_cmd(model) do
    list_servers =
      if Enum.count(model.data["servers"]) > 0 and model.menu == false do
